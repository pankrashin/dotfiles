...
def handle_result(
    ...
    is_nvim = any(
        proc.get("cmdline")
        and any("nvim" in cmd or "vim" in cmd for cmd in proc["cmdline"])
        for proc in fg_processes
    )

    if is_nvim:
        # Send ZZ command (save and quit)
        # This is a normal mode command, no command line shown
        # \x1b = ESC to ensure normal mode
        # ZZ = save and quit (like :wq)
        window.write_to_child("\x1bZZ".encode("utf-8"))
    else:
